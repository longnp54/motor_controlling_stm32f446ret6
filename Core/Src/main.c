...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define ENCODER_CPR 515  // Update based on your measured value (1040-1054)
/* USER CODE END PD */
...
float get_rpm(void);
void motor_forward_speed(uint8_t speed_percent);
void motor_backward_speed(uint8_t speed_percent);
void motor_startup(uint8_t direction);
/* USER CODE END PFP */
...
/* USER CODE BEGIN 4 */
// ✅ HÀM TEST MOTOR 100% VÀ ĐỌC RPM
void test_motor_100_percent(void) {
    static uint32_t last_rpm_update = 0;
    uint32_t current_time = HAL_GetTick();
    
    // ✅ CHẠY MOTOR 100% FORWARD
    Motor_SetDutyCycle(&motor1, 100);
    
    // ✅ ĐỌC RPM mỗi 100ms
    if (current_time - last_rpm_update >= 100) {
        // Đọc encoder counter
        int32_t curr = __HAL_TIM_GET_COUNTER(&htim2);
        int32_t diff = curr - motor1.last_count;
        
        // Tính RPM
        uint32_t dt = current_time - last_rpm_update;
        motor1.current_rpm = ((float)diff * 60000.0f) / (ENCODER_CPR * dt);
        motor1.last_count = curr;
        last_rpm_update = current_time;
        
        // ✅ CẬP NHẬT BIẾN RPM GLOBAL
        current_rpm = motor1.current_rpm;
        
        // Đặt breakpoint ở đây để xem current_rpm
        // current_rpm sẽ hiển thị RPM realtime
    }
}
